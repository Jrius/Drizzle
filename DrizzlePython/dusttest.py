...
ageToLinkToAttr = ptAttribString(2, 'agetolinkto')
spawnpointAttr = ptAttribString(3, 'spawnpoint')
#responder = ptAttribResponder(2, 'responder')
...
class dusttest(ptResponder,):
    ...
    def __init__(self):
        ...
        self.id = 8282
        self.version = 1
        #PtPrintToScreen("init")

    def OnFirstUpdate(self):
        #PtPrintToScreen("onfirstupdate")
        pass
    ...
    def OnNotify(self, state, id, events):
        if (not PtWasLocallyNotified(self.key)):
            return
        #PtPrintToScreen("onnotify")
        print "Dusttest OnNotify..."
        #PtPrintToScreen(`state`)
        #PtPrintToScreen(`id`)
        #PtPrintToScreen(`events`)
        if(command.value=="volcanolink"):
            print "volcanolink"
            agename="DescentMystV"
        ...
        if(command.value=="opendsntdoor"):
            PtPrintToScreen("opendoor")
            responder.run(self.key)
            PtPrintToScreen("opendoordone")
    ...
    def OnServerInitComplete(self):
        #PtPrintToScreen("osic")
        pass

    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        #PtPrintToScreen("sdlnotify")
        pass
