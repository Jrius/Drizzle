...
intValue = ptAttribInt((SubIDStart + 1), 'Set value', default=1, rang=(-1000, 1000))

class xGlobalSDLActSetter(ptModifier):
    ...
    def OnNotify(self, state, id, events):
        ...
        if CheckNotify(state, id, events):
            ageSDL = PtGetAgeSDL()
            curValue = ageSDL[sdlVar.value][0]
            if (intValue.value != curValue):
                ageSDL[sdlVar.value] = (intValue.value,)
                print ('xGlobalSDLActSetter.OnNotify: set sdl var %s to %d' % (sdlVar.value, intValue.value))
