"""
Todo: sync quakes in the Age instance, by using an object master.

(don't forget to netforce the responders)
#"""
...
class dsntQuake(ptResponder):
    ...
    def QuakeStop(self):
        global quaking
        print 'dsntQuake.QuakeStop: The begins to ramp down towards stopping.'
        quaking = false
        self.NextQuakeDelay()
        rampDownTarget = 0
        #self.RampDown()
    ...
    def RampDown(self):
        ...
        if (currentShake <= rampDownTarget):
            print 'dsntQuake.RampDown: The quake reaches it\'s new dampered strength of ',
            print rampDownTarget
            currentShake = rampDownTarget
        else:
            currentShake = (currentShake - 0.0050000000000000001)
            PtAtTimeCallback(self.key, 0.01, 6)
        print 'dsntQuake.RampDown: currentShake = ',
        print currentShake
        if (currentShake == 0):
            print 'dsntQuake.RampDown: Turning the camera shake COMPLETELY off.'
    ...
    def OnSDLNotify(self, VARname, SDLname, playerID, tag):
        global quaking
        ageSDL = PtGetAgeSDL()
        print 'dsntQuake.OnSDLNotify: CanQuake = ',
        print ageSDL[sdlCanQuake.value][0],
        print ' quaking = ',
        print quaking
        if quaking:
    ...
    def BeginAgeUnLoad(self, avObj):
        global quaking
        try: PtGetLocalAvatar()
        except: return
        if (avObj != PtGetLocalAvatar()):
            return
        if quaking:
            print 'dsntQuake.OnLinkFX: The earthquakes stop as you leave the age of Descent.'
            respBaseStop.run(self.key)
            respPeakStop.run(self.key)
        else:
            print 'dsntQuake.OnLinkFX: It wasn\'t quaking when you left the age of Descent.'
