...
//else System.err.println("Shifting on token " + tokStream.getName(lastToken) + " to state " + prs.originalState(currentAction));              
    ...
    private int backtrackParse(IntSegmentedTuple action, int initial_token)
    {
        ...
        //dustin
        int numrounds = 0;
        if(pythondec3.stats.printcurtoken)
        {
            pythondec3.stats.resetCurtoken();
        }
        //end dustin
        for (;;)
        {
            ...
            if(pythondec3.stats.printcurtoken)
            {
                int curtoken = tokStream.peek();
                int curkind = tokStream.getKind(curtoken);
                //shared.m.msg("curtoken: "+Integer.toString(curtoken)+" kind:"+Integer.toString(curkind) + " act:"+Integer.toString(act));
                pythondec3.stats.addCurtoken(curtoken);
            }
            //if(curtoken==113)
            //{
            //    int dummy=0;
            ...
            //if(act<=NUM_RULES)
            //{
            //    pythondec3.stats.simplereduce(act,curtoken);
            //}
            //end dustin
            ...
            {
                //float ratio = (float)curtoken/(float)numrounds;
                //shared.m.msg("Parsing efficiency: "+Float.toString(ratio));
                int curtoken = tokStream.peek();
                pythondec3.stats.efficiency(curtoken, numrounds, "");
